...
//! L4/L7 forwarding, VIP management, service mesh, and SD-WAN operations.
...
async fn main() -> anyhow::Result<()> {
    ...
    let args = Args::parse();
    info!(socket = %args.socket, "Starting novaedge-dataplane");

    // Create in-memory map manager.
    let map_manager = maps::MapManager::new();
    ...
    // Spawn the flow reader task (always mock mode — no eBPF ring buffer).
    let flow_tx_clone = flow_tx.clone();
    let flow_reader_handle = tokio::spawn(async move {
        flows::run_flow_reader(flow_tx_clone).await;
    });
