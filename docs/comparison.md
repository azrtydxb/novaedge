...
NovaEdge consolidates multiple Kubernetes infrastructure tools into a single, unified solution. Instead of deploying and maintaining separate projects for ingress, load balancing, service mesh, WAF, rate limiting, and certificate management, NovaEdge provides all of these capabilities in one binary with one configuration model.
...
**Result**: 5+ separate projects, dozens of CRDs, multiple configuration languages, independent upgrade cycles, and complex failure modes.
...
| **cert-manager** | Automated TLS certificate lifecycle (ACME, Vault, self-signed) | NovaEdge built-in ACME client (HTTP-01, DNS-01, TLS-ALPN-01 with Cloudflare/Route53/Google), cert-manager integration for existing deployments, HashiCorp Vault integration via `ProxyCertificate` CRD |
...
| Feature | NovaEdge | NGINX | Envoy + Istio | Traefik | HAProxy |
|---|:---:|:---:|:---:|:---:|:---:|
| **L7 HTTP/HTTPS Load Balancing** | Yes | Yes | Yes | Yes | Yes |
| **L4 TCP/UDP Proxying** | Yes | Limited | Yes | Yes | Yes |
| **eBPF/XDP Acceleration (via NovaNet)** | Yes | No | No | No | No |
...
| **CRDs to Manage** | 12 | 5+ | 50+ | 20+ | N/A |
| **Components to Deploy** | 2 | 1+ | 5+ | 1 | 1 |

## Operational Benefits

### Single Binary, Single Config Model

NovaEdge uses 12 purpose-built CRDs to express the full configuration surface. A traditional stack combining NGINX Ingress, cert-manager, and Istio requires understanding and maintaining 40+ CRDs across different API groups, each with its own versioning, documentation, and upgrade path.

```
Traditional stack CRDs:  NGINX (5) + cert-manager (6) + Istio (30+) = 41+ CRDs
NovaEdge CRDs:           12 total
```

### Single Observability Pipeline

All NovaEdge metrics are exposed on a single Prometheus endpoint per agent. Request counts, latencies, upstream RTT, active connections, certificate expiry, and WAF blocks are all available from one scrape target. OpenTelemetry traces follow the full request path from ingress through routing and policy enforcement to the upstream response, without requiring trace context propagation across separate proxy boundaries.
...
With NovaEdge, one set of Kubernetes RBAC rules governs access to all networking configuration. In a traditional stack, separate RBAC policies are needed for Ingress resources, cert-manager certificates, Istio policies, and external WAF rules. Consolidation into a single API group simplifies access control and audit logging.

### Fewer Failure Modes

Each additional component in the networking stack introduces its own failure modes, version compatibility requirements, and upgrade risks. Debugging requires correlating logs across multiple components with different log formats and verbosity settings. NovaEdge centralizes these functions, so a single structured log stream and one set of health checks covers the entire data path.
...
```mermaid
flowchart LR
    CLIENT["Client"] --> NGINX["NGINX Ingress<br/>Controller"]
...
**Hops**: Client -> NGINX (L7 routing, TLS, WAF, auth) -> Envoy sidecar (mTLS, policy) -> Application

- 2 proxy hops before reaching the application
...
```mermaid
flowchart LR
    CLIENT["Client"] --> AGENT["NovaEdge Agent<br/>(L7 routing +<br/>policy + mTLS)"]
...
**Hops**: Client -> NovaEdge Agent (L7 routing + TLS + WAF + auth + mTLS) -> Application

- 1 proxy hop before reaching the application
- L7 routing, TLS termination, WAF, authentication, rate limiting, and upstream mTLS all happen in a single process
...
| Memory copies per request | 4+ | 2 |
| Kernel-bypass acceleration | No | Yes (via NovaNet eBPF, when available) |
| Components in failure domain | 2 | 1 |
| Log sources to correlate | 2+ | 1 |
