...
func (h *BGPHandler) AddVIP(ctx context.Context, assignment *pb.VIPAssignment) error {
	...
	if h.bgpServer == nil {
		...
		// BGP server already running — add any new peers from this VIP.
		...
		// The VIP manager sorts VIPs by LocalAS descending before processing,
		// ensuring the VIP with the highest (per-node eBGP) AS starts the server.
