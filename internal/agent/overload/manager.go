...
// OverloadConfig configures the overload manager.
...
// DefaultOverloadConfig returns sensible defaults for the overload manager.
...
// OverloadState tracks current resource pressure.
...
func NewManager(config OverloadConfig, logger *zap.Logger) *Manager {
	return &Manager{
		config: config,
...
func (m *Manager) GetState() OverloadState {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.state
}
...
func (m *Manager) checkResources() {
	...
	if m.state.IsOverloaded {
		...
		if heapRatio >= m.config.MemoryThreshold {
			reason = "memory"
		} else if goroutines >= m.config.GoroutineThreshold {
			reason = "goroutines"
		} else if conns >= m.config.MaxActiveConnections {
			reason = "connections"
		}
